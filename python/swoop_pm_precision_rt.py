...
import math
import random 
import time # for precision timing
...
class Bird:
    ...
    def __init__(self, bird_image_surf, gull_image_surf, screen_w, screen_h, speed_px_s=200.0):
        ...
        # Timing state
        self.spawn_time     = None   # high-res start timestamp (perf_counter)
    ...
    def reset(self):
        ...
        self.spawn_time = time.perf_counter()  # high-res trial start
    ...
    def register_response(self, resp_type):
        ...
        # mark raw response with high-res RT
        self.raw_response = resp_type
        if self.spawn_time is not None:
            self.raw_response_time = time.perf_counter() - self.spawn_time
        else:
            # Fallback; shouldn't occur if reset() was called properly
            self.spawn_time = time.perf_counter()
            self.raw_response_time = 0.0
    ...
    def update(self, dt):
        ...
        """
        # Update trial time using high-res monotonic clock
        if self.spawn_time is not None:
            elapsed = time.perf_counter() - self.spawn_time
        else:
            # Fallback (shouldn't happen, but be safe)
            elapsed = 0.0
            self.spawn_time = time.perf_counter()
        self.t_since_spawn = elapsed
        ...
        if self.raw_response is None and r2 <= inner2:
            self.raw_response_phase = "inner"
            self.t_inner_cross = self.t_since_spawn  # perf-based
